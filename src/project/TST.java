...
public class TST<Value> {
    ...
    **************************************************************/
    ...
    public static void main(String[] args) {
    	 ...
    	 In in;
    	 int pcount=0,ccount=0,ppicount=0,predictioncount=0,bycount=0,thecount=0;
         String urlName = "D:\\1WINDSOR\\workspace-oxygen\\Assignment_4\\src\\textprocessing\\Protein.txt";
         try{
        	 String[] keys = {"protein","complex","PPI","prediction","goals","proteomics"}; 
        	 in = new In(urlName);
        	 String txt = in.readAll();
        	 String[] s3=txt.split(" ");
        	 for(int i=0;i<s3.length;i++){
        		 if(keys[0].equals(s3[i])){
        			 pcount++;
        		 }
        	 }
        	 for(int i=0;i<s3.length;i++){
        		 if(keys[1].equals(s3[i])){
        			 ccount++;
        		 }
        	 }
        		 
        		 for(int i=0;i<s3.length;i++){
        			 if(keys[2].equals(s3[i])){
        				 ppicount++;
        			 }
        		 }
        		 for(int i=0;i<s3.length;i++){
        			 if(keys[3].equals(s3[i])){
        				 predictioncount++;
        			 }
        		 }
        		 for(int i=0;i<s3.length;i++){
        			 if(keys[4].equals(s3[i])){
        				 bycount++;
        			 }
        		 }
        		 for(int i=0;i<s3.length;i++){
        			 if(keys[5].equals(s3[i])){
        				 thecount++;
        			 }
        		 }
        	 ...
        	 /*for(int i=0;i<s3.length;i++){
        		 ...
        		 System.out.println();
            	 System.out.println("'"+keys[0]+"' occurs ("+pcount+") times");
            	 System.out.println("'"+keys[1]+"' occurs ("+ccount+") times");
            	 System.out.println("'"+keys[2]+"' occurs ("+ppicount+") times");
            	 System.out.println("'"+keys[3]+"' occurs ("+predictioncount+") times");
            	 System.out.println("'"+keys[4]+"' occurs ("+bycount+") times");
            	 System.out.println("'"+keys[5]+"' occurs ("+thecount+") times");
         }
         catch(Exception e){
        	 e.printStackTrace();
         }
